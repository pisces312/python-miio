...
class MiCloud:
    ...
    def _login_step2(self, sign):
        ...
        # Handle CAPTCHA + 2FA loop
        max_retries = 5
        for _ in range(max_retries):
            # Handle CAPTCHA (may need multiple attempts)
            while True:
                ...
                print(f"请查看图片并输入验证码: ", end="", flush=True)
                captcha_code = input().strip()
                post_data['captCode'] = captcha_code
                response = self.session.post(url, data=post_data)
                response_json = json.loads(response.text.replace("&&&START&&&", ""))
                logging.debug("Xiaomi login step 2 captcha retry: %s", json.dumps(response_json))

            # Check if we got tokens (login success)
            if 'userId' in response_json and 'ssecurity' in response_json:
                break

            # Handle 2FA: notificationUrl present but tokens missing
            ...
            if notification_url:
                ...
                print(f"\n验证完成后，按回车键继续...")
                input()
                # Visit notificationUrl in our session to sync 2FA state
                try:
                    self.session.get(notification_url)
                except Exception:
                    pass
                # Retry step2 with same session
                response = self.session.post(url, data=post_data)
                response_json = json.loads(response.text.replace("&&&START&&&", ""))
                logging.debug("Xiaomi login step 2 after 2FA: %s", json.dumps(response_json))
                continue

            raise MiCloudAccessDenied("Access denied. Did you set the correct api key and/or username?")

        if response_json.get('result') != "ok" or 'userId' not in response_json:
            raise MiCloudAccessDenied("Access denied. Did you set the correct api key and/or username?")
